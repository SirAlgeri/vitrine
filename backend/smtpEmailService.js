...
// Criar transporter SMTP com configurações do tenant
async function createTransporter(config) {
  // Se config tem SMTP configurado, usar do banco
  ...
  // Fallback: usar variáveis de ambiente (para compatibilidade)
...
export async function sendEmail(to, subject, htmlBody, config = {}) {
  try {
    const transporter = await createTransporter(config);
    
    const fromName = config.smtp_from_name || config.store_name || 'Loja';
...
export async function sendOrderStatusEmail(order, newStatus, config = {}) {
  ...
  return await sendEmail(order.customer_email, subject, htmlBody, config);
}
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f4f4f4;">
  ...
  return await sendEmail(email, subject, htmlBody, config);
}
